...
Maintainer and contributor notes for this plugin. If you are here to use it, the
...
`quality.yml` runs in exactly two places: on demand, and as the release gate. It
has no `pull_request` trigger and no `push` trigger, both deliberately, to keep
Actions minutes for releases. Nothing verifies a pull request or a merge
automatically, so run the relevant checks locally before merging. The hosted gate covers
...
These are the seams paid modules and third-party add-ons attach to. The free
plugin remains the authoritative ledger in every case.
