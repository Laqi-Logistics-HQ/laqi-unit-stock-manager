	...
	public function reserve( int $order_id, array $demand, string $expires_at ): void {
		...
		try {
			foreach ( $demand as $pool_id => $quantity ) {
				...
				$expired = (int) $this->db->get_var( $this->db->prepare( 'SELECT COALESCE(SUM(quantity_available_base),0) FROM ' . $this->db->prefix . 'laqi_lusm_batches WHERE pool_id=%d AND (status IN (%s,%s) OR (status=%s AND expiry_date IS NOT NULL AND expiry_date<%s))', $pool_id, 'quarantined', 'recalled', 'active', current_time( 'Y-m-d' ) ) );
